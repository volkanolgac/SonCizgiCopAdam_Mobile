          ...
          <section key={w.id} className="mt-6 landscape:mt-4">
            ...
            <div className="grid grid-cols-5 gap-2 mt-3">
              {LEVELS.filter((l) => l.world === w.id).map((l) => {
                ...
                if (locked) {
                  return (
                    <div key={l.id} className="level-tile locked" aria-label={`Level ${l.id} locked`}>
                      🔒
                    </div>
                  );
                }
